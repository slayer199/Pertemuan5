...
app.get("/test-connection", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Koneksi database gagal!", error: error.message });
  }
});
...
app.post("/api/media", async (req, res) => {
  ...
  if (!judul || !tahun_rilis || !genre) {
    return res
      .status(400)
      .json({ message: "Judul, Tahun Rilis, dan Genre harus diisi" });
  }
  ...
  try {
    ...
    res
      .status(201)
      .json({ id_media: result.insertId, judul, tahun_rilis, genre });
  } catch (error) {
    console.error("Kesalahan Server:", error);
    res.status(500).json({ message: "Kesalahan Server" });
  }
